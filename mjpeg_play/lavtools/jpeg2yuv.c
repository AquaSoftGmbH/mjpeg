...
static int generate_YUV4MPEG(parameters_t *param)
{
  ...
  for (frame = param->begin;
	...
	mjpeg_info("Rewriting latest frame instead.");
      ...
      if ((param->interlace == Y4M_ILACE_NONE) || (param->interleave == 1)) {
	mjpeg_info("Processing non-interlaced/interleaved %s, size %u.", 
		   jpegname, jpegsize);
	...
	switch (param->interlace) {
	case Y4M_ILACE_TOP_FIRST:
	  mjpeg_info("Processing interlaced, top-first %s, size %u.",
		     jpegname, jpegsize);
	  decode_jpeg_raw(jpegdata, jpegsize,
			  LAV_INTER_TOP_FIRST,
	  ...
	  break;
	case Y4M_ILACE_BOTTOM_FIRST:
	  mjpeg_info("Processing interlaced, bottom-first %s, size %u.", 
		     jpegname, jpegsize);
	  decode_jpeg_raw(jpegdata, jpegsize,
