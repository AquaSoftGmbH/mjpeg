...
void streamout(void)
{
   ...
   if (param.scenefile)
   {
      ...
      /* Increase speed */
      param.output_width = param.output_width/param.scene_detection_decimation;
   ...
   for (framenum = param.offset; framenum < (param.offset + param.frames); ++framenum) {
      readframe(framenum, buffer.frame_buf, &bounds, &param, &buffer, el);
      if (param.scenefile) {
         lum_mean =
             luminance_mean(buffer.frame_buf[0], param.output_width, param.output_height);
         if (framenum == 0)
         {
            delta_lum = 0;
