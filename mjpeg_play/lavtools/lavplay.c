...
void Usage(char *progname)
{
   ...
   fprintf(stderr, "   -v num     Level of verbosity. 0 = quiet, 1 = info, 2 = debug\n");
   fprintf(stderr, "   -o [np]    Use norm NTSC or PAL (default: guess it from framerate\n");
   fprintf(stderr, "   -X num     Horizontal offset\n");
   fprintf(stderr, "   -Y num     Vertical offset\n");
...
int main(int argc, char ** argv)
{
   ...
   */
	...
	while( (n=getopt(argc,argv,"v:a:X:Y:s:c:n:t:qSZHCxzg")) != EOF)
	{
		...
		case 'v' :
			verbose = atoi(optarg);
			if( verbose < LOG_DEBUG && verbose > LOG_ERROR )
			{
		...
		case 'H':
            ...
            break;
		case 'C':
			mjpeg_info("Choosing hardware MJPEG playback (on-card)\n");
            screen_output = 0;
			soft_play = 0;
            break;
		}
		...
		if(res>0)
		{
			...
			case 'o':
				...
				if(input_buffer[1]=='m')
				{
					...
					arguments[0] = movie;
					mjpeg_debug("Opening %s\n", movie);
					if (nc1!=-1) mjpeg_debug(" (frames %d-%d)\n", nc1, nc2);
