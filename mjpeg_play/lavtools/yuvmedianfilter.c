...
main(int argc, char *argv[])
{
   ...
   (void)mjpeg_default_handler_verbosity(verbose);
	...
	mjpeg_debug( "width=%d height=%d rate=%d\n", horz, vert, frame_rate);
	input_frame[0] = alloca(horz * vert);
	input_frame[1] = alloca(horz * vert);
	input_frame[2] = alloca(horz * vert);

	output_frame[0] = alloca(horz * vert);
	output_frame[1] = alloca(horz * vert);
	output_frame[2] = alloca(horz * vert);
...
read_yuv(int fd, int *horizontal, int *vertical, int *frame_rate_code)
{
	...
	for(n=0; n<255; n++) {
		if(!read(fd, &buffer[n], 1)) {
			mjpeg_error_exit1("Error reading header\n");
			exit(1);
		}
	...
	if(buffer[n] != '\n') {
		mjpeg_error_exit1("Didn't get linefeed in first %d characters of data\n",
				255);
		exit(1);
	}
	...
	{
		mjpeg_error_exit1("Input starts not with \"YUV4MPEG\"\n");
		mjpeg_error_exit1("This is not a valid input for me\n");
		exit(1);
	}
...
read_frame(int fd, int chroma, int width, int height, unsigned char *frame[])
{
	...
	if (memcmp(buffer, "FRAME\n", 6)) {
		mjpeg_error_exit1( "Start of new frame missing (%.6s\n", buffer);
	}
	...
	for(i=0; i < v; i++) {
		if (piperead(fd, frame[0] + i * width, h) != h) {
			mjpeg_error_exit1( "EOF while reading luma data\n");
		}
	}
	...
	for(i=0; i < v; i++) {
		if (piperead(fd, frame[1] + i * h, h) != h) {
			mjpeg_error_exit1( "EOF while reading chroma u data\n");
		}
	}
	for(i=0; i < v; i++) {
		if (piperead(fd, frame[2] + i * h, h) != h) {
			mjpeg_error_exit1( "EOF while reading chroma v data\n");
		}
	}
...
write_frame(int fd, int chroma, int width, int height, unsigned char *frame[])
{
	...
	if (write(fd, "FRAME\n", 6) != 6) {
		mjpeg_error_exit1( "Can not write Frame to output\n");
	}
	...
	for(i=0; i < v; i++) {
		if (write(fd, frame[0] + i * width, h) != h) {
			mjpeg_error_exit1( "Error writting reading luma data\n");
		}
	}
	...
	for(i=0; i < v; i++) {
		if (write(fd, frame[1] + i * h, h) != h) {
			mjpeg_error_exit1( "EOF while writting chroma u data\n");
		}
	}
	for(i=0; i < v; i++) {
		if (write(fd, frame[2] + i * h, h) != h) {
			mjpeg_error_exit1( "EOF while writting chroma v data\n");
		}
	}
