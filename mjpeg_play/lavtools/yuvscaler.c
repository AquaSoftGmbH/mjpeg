...
{
  fprintf (stderr,
	   "usage: %s  -I [input_keyword] -M [mode_keyword] -O [output_keyword] [-n p|s|n] [-d] [-h] \n"
	   "%s downscales arbitrary sized yuv frames (from stdin) to a specified format (to stdout)\n\n"
	   "%s is keyword driven :\n"
	   "\t INFILE_<name> if input yuv frames come from Editlist files of name <name> (max MAX_EDIT_LIST_FILES files, stdin by default)\n"
...
main (int argc, char *argv[])
{
  ...
  char *filename[MAX_EDIT_LIST_FILES];
  ...
  // Check for the INFILE_ keyword, indicating an input from file
  for (i = 1; i < argc; i++)
    {
      if (strncmp (argv[i], INFILE_KEYWORD, 7) == 0)
	{
	  // Data will come from file, not stdin
	  filename[infile] = argv[i] + 7;
	   ...
	   unsigned int *width_coeff)
{
  ...
  if ((output_height_slice != 1) || (input_height_slice != 1))
	...
	{
	  u_c_p =
	    input + (2 * (out_line_slice / 2) * input_height_slice +
		     out_line_slice % 2) * input_width;
	  for (in_line = 0; in_line < input_height_slice; in_line++)
	    {
	      input_line_p[in_line] = u_c_p;
	      u_c_p += input_width * 2;	// Interlaced image
	    }
	  u_c_p =
	    output + (2 * (out_line_slice / 2) * output_height_slice +
		      out_line_slice % 2) * output_width;
	  for (out_line = 0; out_line < output_height_slice; out_line++)
	    {
	      output_line_p[out_line] = u_c_p;
	      u_c_p += output_width * 2;	// Interlaced image
	    }
		    ...
		    unsigned int *height_coeff, unsigned int *width_coeff)
{
  ...
  // Interlace is of no importance here
	    ...
	    unsigned int *height_coeff, unsigned int *width_coeff)
{
  ...
  if ((output_height_slice != 1) || (input_height_slice != 1))
	   ...
	   out_line_slice++)
	{
	  u_c_p =
	    input + (2 * (out_line_slice / 2) * input_height_slice +
		     out_line_slice % 2) * input_width / 2;
	  for (in_line = 0; in_line < input_height_slice; in_line++)
	    {
	      input_line_p[in_line] = u_c_p;
	      u_c_p += input_width;	// Interlaced image
	    }
	  u_c_p =
	    output + (2 * (out_line_slice / 2) * output_height_slice +
		      out_line_slice % 2) * output_width / 2;
	  for (out_line = 0; out_line < output_height_slice; out_line++)
	    {
	      output_line_p[out_line] = u_c_p;
	      u_c_p += output_width;	// Interlaced image
	    }
