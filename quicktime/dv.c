...
/*
 *  Specifications of Consumer-Use Digital VCRs using 6.3mm magnetic tape; HD Digital
 *  VCR Conference, December 1994. According to DVCentral, the Blue Book is available
 *  from: 
 *
 *
 *       M. Tsunoo 
...
{
	int i;
	quicktime_dv_codec_t *codec = &(vtrack->codecs.dv_codec);
...
// decompression
	...
	codec->bytes_per_line = (vtrack->track->tkhd.track_width + 3) / 4;
	codec->bytes_per_line *= 6;

	codec->work_buffer = malloc(codec->bytes_per_line *
							vtrack->track->tkhd.track_height);
}

int quicktime_delete_codec_dv(quicktime_video_map_t *vtrack)
{
	free(vtrack->codecs.dv_codec.work_buffer);
}

int quicktime_decode_dv(quicktime_t *file, unsigned char **row_pointers, int track)
{
	long bytes, x, y;
...
bytes = codec->bytes_per_line * vtrack->track->tkhd.track_height;
...
int quicktime_encode_dv(quicktime_t *file, unsigned char **row_pointers, int track)
{
	...
	unsigned char *buffer = vtrack->codecs.dv_codec.work_buffer;
