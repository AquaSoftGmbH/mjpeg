...
/* Init huffman cache */
	memset((char *)htable->cache, 0, ((1 << HUFF_LOOKAHEAD) * sizeof(unsigned QUICKTIME_INT16)));
	p = 0;
	for (l = 1; l <= HUFF_LOOKAHEAD; l++) 
	{
		for (i = 1; i <= (unsigned long) htable->bits[l]; i++, p++) 
		{
			QUICKTIME_INT16 the_code = (unsigned QUICKTIME_INT16)((l << 8) | htable->vals[p]);
				...
				unsigned char *OBuf)
{
	...
	long i, dcval, level;
  	...
  	unsigned QUICKTIME_INT16 *huff_tbl = huff_hdr->cache;
...
/* clear rest of dct buffer */
  	memset((char *)(dct_buf), 0, (DCTSIZE2 * sizeof(QUICKTIME_INT16)));
  	dcval *= (long)qtab[0];
  	dct_buf[0] = (QUICKTIME_INT16)dcval;
  	c_cnt = 0;
  	...
  	{ 
    	...
    	if(size)
    	{
     		...
     		if(coeff)
      		{ 
				c_cnt++;
       			dct_buf[pos] = (QUICKTIME_INT16)(coeff);
      		}
     		i++;
   		}
    	else
