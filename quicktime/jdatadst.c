...
// Set the pointer to the preallocated buffer
  ...
  dest->pub.free_in_buffer = dest->engine->output_size;
}
...
// Just get the length
	my_dest_ptr dest = (my_dest_ptr) cinfo->dest;
	dest->engine->image_size = dest->engine->output_size - dest->pub.free_in_buffer;
}
