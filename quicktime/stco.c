...
int quicktime_update_stco(quicktime_stco_t *stco, long chunk, long offset)
{
	...
	if(chunk > stco->entries_allocated)
	{
		...
		new_table = (quicktime_stco_table_t*)malloc(sizeof(quicktime_stco_table_t) * stco->entries_allocated);
		for(i = 0; i < stco->total_entries; i++) new_table[i].offset = stco->table[i].offset;
		free(stco->table);
