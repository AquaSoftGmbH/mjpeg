...
{
	quicktime_twos_codec_t *codec = &(file->atracks[track].codecs.twos_codec);
...
{
	quicktime_twos_codec_t *codec = &(atrack->codecs.twos_codec);
	codec->work_buffer = 0;
	codec->buffer_size = 0;
	return 0;
}

int quicktime_delete_codec_twos(quicktime_audio_map_t *atrack)
{
	quicktime_twos_codec_t *codec = &(atrack->codecs.twos_codec);
...
int quicktime_decode_twos(quicktime_t *file, 
...
int quicktime_encode_twos(quicktime_t *file, 
