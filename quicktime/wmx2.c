...
int wmx2_decode_block(quicktime_audio_map_t *atrack, QUICKTIME_INT16 *output, unsigned char *input, int samples)
{
	int predictor;
	int index;
	int step;
	...
	QUICKTIME_INT16 *output_end = output + samples;
...
int wmx2_encode_block(quicktime_audio_map_t *atrack, unsigned char *output, QUICKTIME_INT16 *input, int step, int channel, int samples)
{
	quicktime_wmx2_codec_t *codec = &(atrack->codecs.wmx2_codec);
...
// Get the buffer to read into.
	...
	if(!codec->write_buffer)
	{
		...
		codec->write_buffer = malloc(sizeof(QUICKTIME_INT16) * codec->write_size);
	}
