...
PKG_NAME="Video/GIMP (GAP)"
ACLOCAL_FLAGS=""
ACLOCAL_AUXDIR="/usr/local/share/aclocal"

(test -f $srcdir/configure.in \
  && test -d $srcdir/video_enc) || {
    echo -n "**Error**: Directory "\`$srcdir\'" does not look like the"
    echo " top-level gap directory"
...
do 
  ...
  if test -f $dr/NO-AUTO-GEN; then
    ...
    echo processing $dr
# gzchange    macrodirs=`sed -n -e 's,AM_ACLOCAL_INCLUDE(\(.*\)),\1,gp' < $coin`
  macrodirs=$ACLOCAL_AUXDIR
    ( cd $dr
